...
auto FindSplitTunnelDevice()
{
	return FindFirstDevice(&WFP_CALLOUTS_CLASS_ID, SPLIT_TUNNEL_DEVICE_NAME);
}
...
ReturnCode CommandSplitTunnelEvaluate(const std::vector<std::wstring> &args)
{
	...
	auto optDevice = FindSplitTunnelDevice();
	...
	auto device = optDevice.value();

	common::memory::ScopeDestructor dtor;

	dtor += [&device]()
	{
		SetupDiDestroyDeviceInfoList(device.deviceInfoSet);
	};
