...
{
	auto deviceInfoSet = SetupDiGetClassDevsW(deviceClass, nullptr, nullptr, DIGCF_PRESENT);
	...
	while (SetupDiEnumDeviceInfo(deviceInfoSet, deviceIndex, &deviceInfo))
	{
		++deviceIndex;

		auto candidateDeviceName = GetDeviceStringProperty(deviceInfoSet, deviceInfo, &DEVPKEY_NAME);

		if (0 != candidateDeviceName.compare(deviceName))
		{
			continue;
		}
...
)
{
	...
	(
		...
		const_cast<PSP_DEVINFO_DATA>(&deviceInfo.deviceInfo),
		0,
		&needReboot
	);
