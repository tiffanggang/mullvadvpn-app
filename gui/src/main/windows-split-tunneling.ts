...
export async function getApplications(options: {
  ...
  return getApplicationsImpl(options);
}

async function getApplicationsImpl(options: {
  applicationPaths?: string[];
  noCache?: boolean;
}): Promise<IApplication[]> {
  const shortcuts = Object.values(shortcutCache)
    .filter(
      (shortcut) =>
        ...
        options.applicationPaths.includes(shortcut.target),
    )
    .sort((a, b) => a.name.localeCompare(b.name));

  return Promise.all(
    shortcuts.map(async (shortcut) => {
      if (options.noCache || applicationCache[shortcut.target] === undefined) {
        const application = await convertToSplitTunnelingApplication(shortcut);
        applicationCache[shortcut.target] = application;
        return application;
      } else {
